...
mlq *ml_queue[4] ; // TODO allocate memory 
                    ...
                    void *(*function)(void *), void *arg)
{
    ...
    if (first_time_creating){
        ...
        printf("swaping to shedule\n");
        // swapcontext(current_thread_tcb->t_context, uctx_sched);
        schedule();
    }
    // schedule();
    return 0;
...
// Deallocated any dynamic memory created when starting this thread
    ...
    */
    ...
    tcb * found_waiting_thread =  search_for_waiting(ptr->tid);
    ...
    // No need to add to list of terminated b/c wont  need to referenced return value cause its NULL
    if(value_ptr == NULL){
        ...
        current_thread_tcb->t_status = TERMINATED;
        // printf("printing head of finsihed thread: %d", exited_threads_head->finished_thread_tcb->tid);
        rpthread_yield();
    }
...
static void schedule()
{
    ...
    // {
        ...
        // printf("should be printed twice(shed)*:  %d \n", x);
        // tcb * schedule_thread = findSmallest();
        // if(schedule_thread->t_status == TERMINATED){
        //     // remove_from_list(schedule_thread->tid);
        ...
        // setitimer(ITIMER_PROF, &mytime, NULL);
        // printf("should be printed twice(shed)***:  %d \n", x);
        // printf("schedule thread data = %d , %d , %d ",schedule_thread->tid, schedule_thread-> quantum, schedule_thread-> t_status);
        // swapcontext(uctx_sched,list_head->t_context);
        // x++;
        // }
        // printf("done with whie\n");
        printf("should be printed twice(beforeshed)***:  %d \n", x);
        printf("***:  %c \n",current_thread_tcb->n );
        tcb * schedule_thread = findSmallest();
        getitimer(ITIMER_PROF, &mytime);
        mytime.it_value.tv_usec=5;
        current_thread_tcb=schedule_thread;
        setitimer(ITIMER_PROF, &mytime, NULL);
        printf("should be printed twice(aftershed)***:  %d \n", x);
        printf("***:  %c \n",schedule_thread->n );
        swapcontext(uctx_sched,schedule_thread->t_context);
        x++;
        // tcb * ptr = list_head;
        // while (ptr!=NULL){
        //     printf("id:  %d",ptr->quantum);
        // }
        // printf("done with whie\n");
        // if (sctf_flag)
        // {
...
void create_scheduler_context()
{
    ...
    void (*scheduler_ptr)(void*) = &schedule;
    makecontext(uctx_sched, (void *)scheduler_ptr, 0);
}
...
void addToFront(tcb *addThis){
    ...
    printf("number of items in TCB_list = %d\n", x);
}
...
tcb * findSmallest(){
    ...
    tcb * ptr, * current_smallest;
    ptr = list_head;
    int smallest;
    smallest = ptr-> quantum;
    current_smallest = ptr;
    
    while(ptr!=NULL){
        if(ptr->t_status !=TERMINATED) return ptr;
        ptr = ptr->next;
...
tcb * search_for_waiting(rpthread_t goal_tid){
    tcb * ptr_queue_head;
    ptr_queue_head = list_head;
    if(ptr_queue_head == NULL) return NULL; // list empty for some reason (should always have main)
...
void *remove_from_list(rpthread_t goal_tid){
    ...
    if(ptr!=NULL && ptr->tid == goal_tid){
        list_head = ptr->next;
        free(ptr);
        return;
    }
    while(ptr!=NULL && ptr->tid != goal_tid){
        prev = ptr; 
        ptr = ptr->next; 
    }
    if (ptr == NULL) return; 
